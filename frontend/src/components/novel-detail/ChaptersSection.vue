...
watch(
  ...
  async (list) => {
    ...
    const focusChapter = props.focusChapterNumber
    if (
      focusChapter &&
      list.some((chapter) => chapter.chapter_number === focusChapter) &&
      selectedChapter.value?.chapter_number !== focusChapter
    ) {
      await loadChapterDetail(focusChapter)
      return
    }
...
watch(
  () => props.focusChapterNumber,
  async (chapterNumber) => {
    if (!chapterNumber || chapters.value.length === 0) {
      return
    }
    if (selectedChapter.value?.chapter_number === chapterNumber) {
      return
    }
    const exists = chapters.value.some((chapter) => chapter.chapter_number === chapterNumber)
    if (!exists) {
      return
    }
