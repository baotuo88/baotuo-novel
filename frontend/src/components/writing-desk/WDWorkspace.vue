      ...
      <div v-if="selectedChapterNumber" class="md-card-header flex-shrink-0">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          ...
          <div class="flex w-full sm:w-auto items-center justify-end flex-wrap gap-2">
            <button
              v-if="project?.id"
              @click="$emit('openWorldGraph')"
              class="md-btn md-btn-outlined md-ripple flex items-center gap-2 whitespace-nowrap"
            >
              <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 20l-5.447-2.724A1 1 0 013 16.382V5.618a1 1 0 01.553-.894L9 2m0 18l6-3m-6 3V2m6 15l5.447 2.724A1 1 0 0021 18.382V7.618a1 1 0 00-.553-.894L15 4m0 13V4m-6-2l6 2" />
              </svg>
              世界图谱
            </button>
            <button
      ...
      <div class="md-card-content flex-1 overflow-y-auto">
        <component
          ...
          @generateChapter="$emit('generateChapter', $event)"
            ...
            @chapter-updated="$emit('chapterUpdated', $event)"
        />
      </div>
    </div>
